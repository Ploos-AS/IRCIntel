...
set -e
cat "$log"
if (( rc != 0 )); then echo "dr_integrity_chain_result=scheduled_failed"; exit "$rc"; fi
...
manifest_archive=$(ops/postgres/dr-evidence-manifest-s3.sh "$manifest")
printf '%s\n' "$manifest_archive"
manifest_key=$(printf '%s\n' "$manifest_archive"|grep '^dr_manifest_archive_key='|tail -1|cut -d= -f2-)
[[ -n "$manifest_key" ]] || { echo "dr_integrity_chain_result=manifest_archive_key_missing" >&2; exit 66; }
sha=$(sha256sum "$record"|awk '{print $1}'); manifest_sha=$(sha256sum "$manifest"|awk '{print $1}')
...
echo "dr_integrity_chain_manifest_archive_key=$manifest_key"
echo "dr_integrity_chain_manifest_archived=true"
