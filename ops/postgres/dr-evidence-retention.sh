...
days=${IRCINTEL_DR_EVIDENCE_RETENTION_DAYS:-365}
...
for value in "$days" "$base_days"; do
  [[ "$value" =~ ^[1-9][0-9]*$ ]] || {
    echo "dr_evidence_retention_result=invalid_days" >&2
    exit 64
  }
done

# Acceptance evidence must outlive the backup window it attests to.
if (( days < base_days )); then
  echo "dr_evidence_retention_result=unsafe_window" >&2
  echo "dr_evidence_retention_days=$days" >&2
  echo "basebackup_retention_days=$base_days" >&2
  exit 65
fi

for cmd in aws python3 mktemp; do
  command -v "$cmd" >/dev/null 2>&1 || { echo "dr_evidence_retention_missing_command=$cmd" >&2; exit 69; }
done
...
print(json.dumps({"Rules":[{
    "ID":"ircintel-dr-evidence-retention",
    "Status":"Enabled",
...
if [[ "$mode" == "apply" ]]; then
  # Preserve unrelated lifecycle rules when adding/updating the evidence rule.
  current=$(aws "${aws_args[@]}" get-bucket-lifecycle-configuration --bucket "$bucket" --output json 2>/dev/null || printf '{"Rules":[]}')
  merged=$(mktemp)
  trap 'rm -f "$policy" "$merged"' EXIT
  python3 - "$current" "$policy" >"$merged" <<'PY'
import json,sys
current=json.loads(sys.argv[1])
wanted=json.load(open(sys.argv[2], encoding='utf-8'))['Rules'][0]
rules=[r for r in current.get('Rules',[]) if r.get('ID') != wanted['ID']]
rules.append(wanted)
...
days=int(sys.argv[3])
matches=[r for r in obj.get('Rules',[]) if r.get('ID')=='ircintel-dr-evidence-retention']
if len(matches) != 1:
    print('dr_evidence_retention_result=mismatch')
    raise SystemExit(74)
r=matches[0]
if (r.get('Status')!='Enabled' or r.get('Filter',{}).get('Prefix')!=prefix or r.get('Expiration',{}).get('Days')!=days):
    print('dr_evidence_retention_result=mismatch')
    raise SystemExit(74)
print('dr_evidence_retention_result=ok')
print(f'dr_evidence_retention_days={days}')
print(f'dr_evidence_retention_prefix={prefix}')
PY
